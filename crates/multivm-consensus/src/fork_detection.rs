...
mod tests {
    ...
    async fn test_fork_info_creation() {
        let fork_info = ForkInfo::new(100, ForkReason::ValidatorDisagreement);
        assert_eq!(fork_info.fork_height, 100);
        assert_eq!(fork_info.status, ForkStatus::Detected);
        assert!(fork_info.id.starts_with("fork-100-"));
    }
