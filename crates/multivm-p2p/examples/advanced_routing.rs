...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut count = 0;
    while let Ok(Some((peer_id, message))) = tokio::time::timeout(
        std::time::Duration::from_millis(100),
        rx.recv()
    ).await {
        count += 1;
        println!("  Message {} routed to peer {}: {:?}", 
            count, peer_id, message.infer_type());
    }
    ...
    router.route_message(custom_message.clone(), routing::RoutingStrategy::Broadcast).await?;
    println!("\nRouted control message with broadcast strategy");
    
    // Example 5: Remove routes
    command_sender.send(routing::RoutingCommand::RemoveRoute {
        message_type: MessageType::Svm,
        peer_id: peer1,
    }).await?;
