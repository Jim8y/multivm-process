...
async fn test_end_to_end_message_flow() {
    ...
    let config = config::P2PConfig::default();
...
async fn test_protocol_translation_integration() {
    ...
    // Translate to EVM
    let evm_translated = translator.translate_message(&svm_message, VmType::Evm).unwrap();
...
async fn test_network_discovery_integration() {
    ...
    command_sender.send(discovery::DiscoveryCommand::AddBootstrapPeer {
        peer_id,
    ...
    match event_result {
        Ok(Some(discovery::DiscoveryEvent::PeerDiscovered { peer })) => {
            assert_eq!(peer.peer_id, peer_id);
            assert!(peer.addresses.contains(&address));
        }
        _ => (), // Timeout or other events are acceptable in integration test
    }
...
async fn test_transport_network_integration() {
    ...
    match event_result {
        Ok(Some(transport::TransportEvent::MessageSent { peer_id: sent_peer, message_id })) => {
            assert_eq!(sent_peer, peer_id);
            assert_eq!(message_id, test_message.id);
        }
        _ => (), // Timeout acceptable in integration test
    }
...
async fn test_routing_integration() {
    ...
    command_sender.send(routing::RoutingCommand::AddRoute {
    ...
    let route_result = router.route_message(
        test_message.clone(),
...
async fn test_performance_characteristics() {
    ...
    assert!(creation_time < Duration::from_millis(100), "Message creation should be fast");
    ...
    let start = Instant::now();
    for _i in 0..100 {
        let _result = translator.translate_message(&test_message, VmType::Evm).unwrap();
    }
    let translation_time = start.elapsed();
    
    // Should be able to translate 100 messages quickly
    assert!(translation_time < Duration::from_millis(500), "Translation should be fast");
}
...
async fn test_full_stack_simulation() {
    ...
    // Routing setup
    let routing_config = routing::RoutingConfig::default();
    let (router, routing_commands) = routing::MessageRouter::new(transport_messages, Some(routing_config));
    ...
    discovery_commands.send(discovery::DiscoveryCommand::AddBootstrapPeer {
        peer_id: remote_peer,
        address: remote_addr.clone(),
    }).await.unwrap();
    
    // Add routing entry
    routing_commands.send(routing::RoutingCommand::AddRoute {
    ...
    // Translate message
    let translated = translator.translate_message(&cross_vm_message, VmType::Evm).unwrap();
    
    // Route message
    let route_result = router.route_message(translated, routing::RoutingStrategy::Broadcast).await;
    assert!(route_result.is_ok());
