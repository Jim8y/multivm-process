...
use crate::messages::{NetworkMessage, MessageType};
...
impl MessageRouter {
                ...
                for (msg_type, routes) in table.iter_mut() {
                    routes.retain(|route| {
                        ...
                        if !is_recent || !is_reliable {
                            debug!("Removing stale route for {:?} to peer {}", msg_type, route.peer_id);
                        }
    ...
    async fn handle_command(&self, command: RoutingCommand) -> Result<()> {
        match command {
            RoutingCommand::AddRoute { message_type, peer_id, address } => {
                self.add_route(message_type, peer_id, address).await?;
            }
            RoutingCommand::RemoveRoute { message_type, peer_id } => {
    ...
    async fn handle_command_static(
        ...
        match command {
            RoutingCommand::AddRoute { message_type, peer_id, address } => {
                Self::add_route_static(message_type, peer_id, address, routing_table, stats).await?;
            }
            RoutingCommand::RemoveRoute { message_type, peer_id } => {
            ...
            RoutingCommand::UpdateReliability { peer_id, success } => {
                Self::update_peer_reliability_static(peer_id, success, routing_table, config).await?;
            }
            RoutingCommand::GetStats(sender) => {
                let stats = Self::get_routing_stats_static(routing_table, subscribed_topics, stats).await;
                let _ = sender.send(stats);
            }
        }
        Ok(())
    ...
    async fn broadcast_message(&self, message: NetworkMessage) -> Result<()> {
        ...
        for routes in table.values() {
            for route in routes {
                if let Err(e) = self.message_sender.send((route.peer_id, message.clone())).await {
    ...
    async fn direct_message(&self, message: NetworkMessage, peer_id: PeerId) -> Result<()> {
        self.message_sender.send((peer_id, message)).await
    ...
    pub async fn has_routes(&self, message_type: &MessageType) -> bool {
        let table = self.routing_table.read().await;
        table.get(message_type).map_or(false, |routes| !routes.is_empty())
    }
    ...
    async fn get_routing_stats_static(
        ...
        current_stats.peer_count = table.values()
            .flat_map(|routes| routes.iter().map(|r| r.peer_id))
            .collect::<HashSet<_>>()
            .len();
