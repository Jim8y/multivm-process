...
mod tests {
    ...
    fn test_metrics_initialization() {
        ...
        record_processing_duration("multivm", 0.05);
        update_active_peers("svm", "connected", 5.0);
        update_bandwidth("upload", "gossipsub", 1024.0);
        record_protocol_translation("svm", "evm", "success");
        record_routing_decision("broadcast", "control", "success");
        record_peer_discovery("mdns", "success");
        update_network_health("overall", 2.0);
        update_connection_pool("tcp", "active", 10.0);
