...
mod tests {
    ...
    fn test_config_validation() {
        ...
        // Reset and test empty listen addresses
        config = P2PConfig::default();
        config.network.listen_addresses.clear();
        assert!(config.validate().is_err());

        // Reset and test no transports enabled
        config = P2PConfig::default();
        config.transport.enable_tcp = false;
        config.transport.enable_quic = false;
    ...
    fn test_config_serialization() {
        ...
        assert_eq!(
            ...
            deserialized.network.max_connections
        );
        assert_eq!(
            config.protocol.current_version,
            deserialized.protocol.current_version
        );
    }

    #[test]
    fn test_log_level_ordering() {
        // Ensure that log levels can be compared if needed
        let info = LogLevel::Info;
        let debug = LogLevel::Debug;

        // At minimum, ensure they can be created and serialized
        let json_info = serde_json::to_string(&info).unwrap();
        let json_debug = serde_json::to_string(&debug).unwrap();

        assert!(json_info.contains("Info"));
        assert!(json_debug.contains("Debug"));
    }
}
