...
async fn test_discovery_service_creation() {
    ...
    let (service, _command_sender, _event_receiver) =
        DiscoveryService::new(Some(config), local_peer_id);
...
async fn test_discovery_service_lifecycle() {
    ...
    let (service, command_sender, mut event_receiver) =
        DiscoveryService::new(Some(config), local_peer_id);
    ...
    assert!(
        start_result.is_ok(),
        "Discovery service should start successfully"
    );
    ...
    assert!(
        stop_result.is_ok(),
        "Discovery service should stop successfully"
    );
...
async fn test_discovery_commands() {
    ...
    let (service, command_sender, mut _event_receiver) =
        DiscoveryService::new(Some(config), local_peer_id);
    ...
    let add_peer_result = command_sender
        .send(DiscoveryCommand::AddBootstrapPeer {
            peer_id,
    ...
    assert!(
        add_peer_result.is_ok(),
        "Add bootstrap peer command should succeed"
    );

    // Test remove peer command
    let remove_peer_result = command_sender
        .send(DiscoveryCommand::RemovePeer { peer_id })
        .await;
    assert!(
        remove_peer_result.is_ok(),
        "Remove peer command should succeed"
    );

    // Test DHT lookup command
    let lookup_result = command_sender
        .send(DiscoveryCommand::DhtLookup {
...
async fn test_discovery_stats() {
    ...
    let (service, command_sender, _event_receiver) =
        DiscoveryService::new(Some(config), local_peer_id);
...
async fn test_discovered_peer_info() {
    ...
    assert!(matches!(
        discovered_peer.connection_status,
...
async fn test_peer_management() {
    ...
    let update_result = service
        .update_peer_status(test_peer_id, ConnectionStatus::Connected)
        .await;
    assert!(
        update_result.is_ok(),
        "Status update should handle missing peer gracefully"
    );
}
...
async fn test_discovery_behaviour_creation() {
    ...
    assert!(
        behaviour_result.is_ok(),
        "Discovery behaviour creation should succeed"
    );
}
...
async fn test_bootstrap_with_peers() {
    ...
    assert!(
        start_result.is_ok(),
        "Service with bootstrap peers should start successfully"
    );
    ...
    assert!(
        discovered_peers.len() <= 2,
        "Bootstrap process should be running"
    );
}
...
async fn test_discovery_events() {
    ...
    let (_service, command_sender, mut event_receiver) =
        DiscoveryService::new(Some(config), local_peer_id);
    ...
    command_sender
        .send(DiscoveryCommand::AddBootstrapPeer { peer_id, address })
        .await
        .unwrap();
    ...
    if let Ok(Some(DiscoveryEvent::PeerDiscovered { peer })) = event_result {
...
async fn test_dht_operations() {
    ...
    let put_result = command_sender
        .send(DiscoveryCommand::PutValue {
            key: b"test_key".to_vec(),
    ...
    let get_result = command_sender
        .send(DiscoveryCommand::GetValue {
            key: b"test_key".to_vec(),
        })
        .await;
    ...
    let find_result = command_sender
        .send(DiscoveryCommand::FindPeers {
