...
async fn test_transport_layer_creation() {
    ...
    let (transport, _message_sender, _event_receiver) = TransportLayer::new(Some(config), local_key.clone());
...
async fn test_connection_management() {
    ...
    assert!(!is_connected_before, "Peer should not be connected initially");
    ...
    assert!(is_connected_after, "Peer should be connected after connect call");
    ...
    assert!(!is_connected_final, "Peer should not be connected after disconnect");
}
...
async fn test_transport_stats() {
    ...
    let (transport, _message_sender, _event_receiver) = TransportLayer::new(Some(config), local_key);
...
async fn test_connection_stats_update() {
    ...
    let update_result = transport.update_connection_stats(
        peer_id,
        1024, // bytes_sent
        512,  // bytes_received
        5,    // messages_sent
...
async fn test_message_sending() {
    ...
    let (transport, message_sender, mut event_receiver) = TransportLayer::new(Some(config), local_key.clone());
    ...
    match event_result {
        Ok(Some(TransportEvent::MessageSent { peer_id: received_peer_id, message_id })) => {
...
async fn test_transport_events() {
    ...
    let (transport, _message_sender, mut event_receiver) = TransportLayer::new(Some(config), local_key.clone());
    ...
    transport.connect_to_peer(peer_id, address.clone()).await.unwrap();
    ...
    match event_result {
        Ok(Some(TransportEvent::ConnectionEstablished { peer_id: event_peer_id, address: event_address })) => {
            assert_eq!(event_peer_id, peer_id);
            assert_eq!(event_address, address);
        }
        _ => (), // Event might not arrive immediately in test environment
    }
...
fn test_transport_stats_serialization() {
    ...
    assert_eq!(deserialized.total_connections_established, stats.total_connections_established);
    assert_eq!(deserialized.total_connections_closed, stats.total_connections_closed);
    assert_eq!(deserialized.total_bytes_sent, stats.total_bytes_sent);
    assert_eq!(deserialized.total_bytes_received, stats.total_bytes_received);
    assert_eq!(deserialized.total_messages_sent, stats.total_messages_sent);
    assert_eq!(deserialized.total_messages_received, stats.total_messages_received);
...
async fn test_concurrent_connections() {
    ...
    let active_connections = transport.get_active_connections().await;
    assert_eq!(active_connections.len(), 5, "All connections should be active");
}
...
async fn test_transport_with_no_addresses() {
    ...
    match start_result {
        Ok(_) => assert!(true),
        Err(_) => assert!(true), // Both outcomes are acceptable
    }
}