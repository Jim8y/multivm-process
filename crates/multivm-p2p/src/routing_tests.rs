...
async fn test_routing_commands() {
    ...
    let add_route_result = command_sender
        .send(RoutingCommand::AddRoute {
    ...
    let remove_route_result = command_sender
        .send(RoutingCommand::RemoveRoute {
            message_type: MessageType::Control,
            peer_id,
        })
        .await;
    assert!(
        remove_route_result.is_ok(),
        "Remove route command should succeed"
    );

    // Test subscribe command
    let subscribe_result = command_sender
        .send(RoutingCommand::Subscribe("test-topic".to_string()))
        .await;
    assert!(subscribe_result.is_ok(), "Subscribe command should succeed");

    // Test unsubscribe command
    let unsubscribe_result = command_sender
        .send(RoutingCommand::Unsubscribe("test-topic".to_string()))
        .await;
    assert!(
        unsubscribe_result.is_ok(),
        "Unsubscribe command should succeed"
    );

    // Test update reliability command
    let reliability_result = command_sender
        .send(RoutingCommand::UpdateReliability {
            peer_id,
            success: true,
        })
        .await;
    assert!(
        reliability_result.is_ok(),
        "Update reliability command should succeed"
    );
...
async fn test_message_routing() {
    ...
    let broadcast_result = router
        .route_message(test_message.clone(), RoutingStrategy::Broadcast)
        .await;
    assert!(broadcast_result.is_ok(), "Broadcast routing should succeed");

    // Test direct routing
    let direct_result = router
        .route_message(test_message.clone(), RoutingStrategy::Direct(peer_id))
        .await;
    assert!(direct_result.is_ok(), "Direct routing should succeed");

    // Test DHT routing
    let dht_result = router
        .route_message(
            test_message.clone(),
            RoutingStrategy::DHT(b"test_key".to_vec()),
        )
        .await;
    assert!(dht_result.is_ok(), "DHT routing should succeed");

    // Test gossip routing
    let gossip_result = router
        .route_message(
            test_message.clone(),
            RoutingStrategy::Gossip("test-topic".to_string()),
        )
    ...
    // Test random routing
    let random_result = router
        .route_message(test_message, RoutingStrategy::Random(1))
        .await;
    ...
    while let Ok(Some(_)) =
        tokio::time::timeout(Duration::from_millis(50), message_receiver.recv()).await
    {
        received_count += 1;
        if received_count >= 3 {
...
fn test_routing_stats_serialization() {
    ...
    assert_eq!(
        deserialized.total_messages_routed,
        stats.total_messages_routed
    );
...
async fn test_sequential_routing() {
    ...
    for i in 0..5 {
        ...
        let result = router
            .route_message(test_message, RoutingStrategy::Broadcast)
            .await;
    ...
    let mut received_count = 0;
    while let Ok(Some(_)) =
        tokio::time::timeout(Duration::from_millis(20), message_receiver.recv()).await
    {
        received_count += 1;
...
async fn test_routing_with_no_peers() {
    ...
    let random_result = router
        .route_message(test_message, RoutingStrategy::Random(5))
        .await;
...
async fn test_router_stats_after_routing() {
    ...
    let (tx, rx) = tokio::sync::oneshot::channel();
    command_sender
        .send(RoutingCommand::GetStats(tx))
        .await
        .unwrap();
    let stats = rx.await.unwrap();
    ...
    assert!(
        stats.total_messages_routed > 0,
        "Should show routed messages"
    );
