...
mod tests {
    ...
    async fn test_load_balancer_round_robin() {
        ...
        let message = NetworkMessage::new(
            crate::messages::MessagePayload::Control(
                crate::messages::ControlMessage::StatusRequest,
            ),
            crate::messages::MessageSource::NetworkLayer,
            crate::messages::MessageTarget::Broadcast,
        );

        // Test round-robin distribution
        let mut selections = Vec::new();
        for _ in 0..6 {
            if let Some(peer) = lb.select_peer(&message, &peers).await.unwrap() {
                selections.push(peer);
            }
        }

        // Should cycle through all peers
    ...
    async fn test_load_balancer_least_connections() {
        ...
        for (i, peer_id) in peers.iter().enumerate() {
            ...
            metrics.active_connections = i as u32; // 0, 1, 2 connections
            lb.update_peer_metrics(*peer_id, metrics).await;
        }
        ...
        // Should select peer with 0 connections
        let selected = lb.select_peer(&message, &peers).await.unwrap();
        assert_eq!(selected, Some(peers[0]));
    }

    #[tokio::test]
    async fn test_load_balancer_health_filtering() {
        let config = LoadBalancerConfig {
            strategy: LoadBalancingStrategy::RoundRobin,
            enable_health_routing: true,
        ...
        for (i, peer_id) in peers.iter().enumerate() {
            let mut metrics = PeerMetrics::new(*peer_id);
            metrics.reliability_score = if i == 0 { 0.5 } else { 0.9 }; // First peer unhealthy
            lb.update_peer_metrics(*peer_id, metrics).await;
        }
        ...
        // Should only select from healthy peers
        for _ in 0..10 {
            let selected = lb.select_peer(&message, &peers).await.unwrap();
            assert!(selected == Some(peers[1]) || selected == Some(peers[2]));
            assert_ne!(selected, Some(peers[0])); // Unhealthy peer should be filtered out
        }
    }
    ...
    async fn test_response_time_tracking() {
        ...
        // Record responses
        lb.record_response(peer_id, Duration::from_millis(100), true)
            .await;
        lb.record_response(peer_id, Duration::from_millis(200), true)
            .await;
