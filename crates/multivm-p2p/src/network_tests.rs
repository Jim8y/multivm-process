...
async fn test_topic_subscription() {
    ...
    assert!(subscribe_result.is_ok(), "Topic subscription should succeed");
    
    // Verify subscription
    let topics = network.get_subscribed_topics().await;
    assert!(topics.contains("test-topic"), "Topic should be in subscribed list");
    
    // Test unsubscribe
    let unsubscribe_result = network.unsubscribe_topic("test-topic").await;
    assert!(unsubscribe_result.is_ok(), "Topic unsubscription should succeed");
    
    // Verify unsubscription
    let topics_after = network.get_subscribed_topics().await;
    assert!(!topics_after.contains("test-topic"), "Topic should be removed from subscribed list");
}
...
async fn test_message_publishing() {
    ...
    let result = network.publish_message("test-topic", test_data.clone()).await;
    ...
    match result {
        ...
        Err(e) => {
            ...
            assert!(e.to_string().contains("Failed to send publish command") || 
                   e.to_string().contains("channel") ||
                   e.to_string().contains("InsufficientPeers"), 
                   "Expected channel, command, or peers error, got: {}", e);
    ...
    assert!(stats.messages_sent > 0, "Message sent count should be updated");
    assert!(stats.bytes_sent >= test_data.len() as u64, "Bytes sent should be updated");
}
...
async fn test_network_stats() {
    ...
    assert_eq!(stats.connected_peers, 0, "Initial connected peers should be 0");
    assert_eq!(stats.messages_sent, 0, "Initial messages sent should be 0");
    assert_eq!(stats.messages_received, 0, "Initial messages received should be 0");
}
...
async fn test_p2p_network_layer_trait() {
    ...
    assert!(peers.len() >= 0); // Should be empty initially
...
async fn test_message_handling() {
    ...
    let handle_result = network.handle_incoming_message(
        test_message,
        "test-peer".to_string(),
    ).await;
...
async fn test_send_to_peer() {
    ...
    let send_result = network.send_to_peer("test-peer".to_string(), test_message).await;
...
fn test_network_config_validation() {
    ...
    assert!(valid_config.enable_mdns == true);
}
...
fn test_network_config_custom() {
    ...
    assert_eq!(custom_config.enable_mdns, false);
}
...
async fn test_error_handling() {
    ...
    let result = network.send_to_peer("invalid-peer-id".to_string(), invalid_message).await;
...
fn test_network_behaviour_creation() {
    ...
    use libp2p::{kad, gossipsub, mdns, identify, ping};
    ...
    assert_eq!(local_peer_id.to_string().len() > 0, true);
}