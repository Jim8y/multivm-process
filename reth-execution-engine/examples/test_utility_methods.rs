use reth_execution_engine::{
    engine::{Transaction, TransactionSignature, U256},
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let test_transactions = vec![
        ...
        Transaction {
            ...
            gas_limit: 21_000,
            to: Some([
                0x1a, 0x2b, 0x3c, 0x4d, 0x5e, 0x6f, 0x70, 0x80, 0x90, 0xa0, 0xb1, 0xc2, 0xd3, 0xe4,
                0xf5, 0x06, 0x17, 0x28, 0x39, 0x4a,
            ]),
    ...
    for (chain_id, chain_name, _) in &test_chains {
        ...
        for (i, tx) in test_transactions.iter().enumerate() {
            ...
            println!(
                "  Transaction {}: {} bytes, type={}, EIP-1559={}",
                i + 1,
                rlp_encoded.len(),
                tx_type,
                supports_eip1559
            ...
            println!(
                "    RLP hex: {}",
                hex::encode(&rlp_encoded[..std::cmp::min(32, rlp_encoded.len())])
            );
            ...
            if tx_type == 2 && supports_eip1559 {
                ...
                assert!(
                    rlp_encoded[0] >= 0xc0,
                    "Legacy transaction should start with RLP list prefix"
                );
