...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 1..=blocks_to_generate {
        ...
        // 生产新区块
        let new_block_hash = engine.produce_block_v3(current_parent_hash, fee_recipient).await?;
        ...
        loop {
            ...
            info!("⏳ 等待区块 #{} 被添加到链上... (当前: {}, 目标: {}, 尝试: {}/{})", 
                  i, current_block, target_block_number, attempts + 1, max_attempts);
    ...
    if final_block >= expected_final_block {
        ...
        warn!("⚠️ 可能有些区块还在同步中: 期望 {}, 实际 {}", expected_final_block, final_block);
    }
    ...
    info!("   - 初始区块号: {}", initial_block);
    info!("   - 最终区块号: {}", final_block);
    info!("   - 实际增长: {} 个区块", final_block.saturating_sub(initial_block));
