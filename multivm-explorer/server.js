...
async function updateNetworkStatus() {
  ...
  // Calculate total transactions across all blocks (not just latest)
  let totalTransactions = 0;
  for (const node of nodeStatuses) {
    if (node.reth.healthy && node.reth.blockNumber) {
      // Get transaction count from this node
      try {
        // Estimate based on average transactions per block
        totalTransactions += node.reth.blockNumber * (node.latestBlock?.transactionCount || 0);
      } catch (error) {
        console.error(`Error calculating transactions for node ${node.id}:`, error);
      }
    }
  }
