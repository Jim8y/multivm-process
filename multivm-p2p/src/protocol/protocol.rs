...
use super::messages::{MessagePayload, NetworkMessage, VmType};
...
impl ProtocolTranslator {
    ...
    pub fn new() -> Self {
        ...
        translator.capabilities.insert(
            VmType::Svm,
            vec![
            ...
            Ok(_source_vm) => {
                ...
                if let Some(capabilities) = self.capabilities.get(&target_vm) {
                    match &message.payload {
                        ...
                        MessagePayload::Custom(_) => true, // Custom messages are always supported
                    }
                } else {
                    false
                }
    ...
    fn determine_source_vm(&self, payload: &MessagePayload) -> Result<VmType, P2PError> {
        match payload {
            ...
            MessagePayload::Custom(_) => Ok(VmType::Svm), // Default custom messages to SVM
        }
    }
...
impl MessageConverter for SolanaToEvmConverter {
    fn convert(
        ...
        converted_message
            .metadata
            .insert("converted_from".to_string(), "solana".to_string());
        converted_message
            .metadata
            .insert("converted_to".to_string(), "ethereum".to_string());
        converted_message.metadata.insert(
    ...
    fn convert_payload(&self, payload: &MessagePayload) -> Result<MessagePayload, P2PError> {
        match payload {
            MessagePayload::Svm(_svm_msg) => {
                ...
                Ok(MessagePayload::MultiVm(
                    super::messages::MultiVmMessage::StateSync {
                        state_root:
...
impl MessageConverter for EvmToSolanaConverter {
    fn convert(
        ...
        converted_message
            .metadata
            .insert("converted_from".to_string(), "ethereum".to_string());
        converted_message
            .metadata
            .insert("converted_to".to_string(), "solana".to_string());
        converted_message.metadata.insert(
            "conversion_time".to_string(),
            chrono::Utc::now().to_rfc3339(),
        );
...
impl MessageConverter for UniversalConverter {
    fn convert(
        ...
        converted_message
            .metadata
            .insert("universal_conversion".to_string(), "true".to_string());
        converted_message
            .metadata
            .insert("target_format".to_string(), target_format.to_string());
        converted_message.metadata.insert(
            "conversion_time".to_string(),
            chrono::Utc::now().to_rfc3339(),
        );
