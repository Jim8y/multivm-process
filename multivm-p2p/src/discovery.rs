...
impl DiscoveryService {
    ...
    pub fn new(
        ...
        local_peer_id: PeerId,
    ) -> (Self, mpsc::Sender<DiscoveryCommand>, mpsc::Receiver<DiscoveryEvent>) {
    ...
    pub async fn start(&self) -> Result<()> {
        ...
        *self.running.write().await = true;
        ...
        let mut command_receiver = self.command_receiver.write().await.take()
            .ok_or_else(|| P2PError::Internal("Discovery service already started".to_string()))?;
    ...
    async fn bootstrap_dht(&self) -> Result<()> {
        info!("Bootstrapping DHT with {} peers", self.config.bootstrap_peers.len());
        ...
        for (peer_id, address) in &self.config.bootstrap_peers {
            ...
            let _ = self.event_sender.send(DiscoveryEvent::PeerDiscovered { peer }).await;
        }
    ...
    async fn refresh_peers(
        ...
        for peer_id in to_remove {
            ...
            let _ = event_sender.send(DiscoveryEvent::PeerLost { peer_id }).await;
    ...
    async fn handle_command(
        ...
        match command {
            ...
            DiscoveryCommand::AddBootstrapPeer { peer_id, address } => {
                Self::add_bootstrap_peer(peer_id, address, discovered_peers, event_sender, stats).await?;
            }
            ...
            DiscoveryCommand::GetStats(sender) => {
                let current_stats = Self::get_current_stats(discovered_peers, active_queries, stats).await;
    ...
    async fn add_bootstrap_peer(
        ...
        let _ = event_sender.send(DiscoveryEvent::PeerDiscovered { peer }).await;
    ...
    async fn put_dht_value(
        ...
        debug!("Putting value in DHT for key: {} (size: {} bytes)", hex::encode(&key), value.len());
        Ok(())
    }
    ...
    pub async fn update_peer_status(&self, peer_id: PeerId, status: ConnectionStatus) -> Result<()> {
        let mut peers = self.discovered_peers.write().await;
