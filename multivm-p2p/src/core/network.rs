...
use crate::secure_network::NetworkStats;
...
impl P2PNetwork {
    ...
    async fn handle_behaviour_event(
        ...
        match event {
            NetworkBehaviourEvent::Gossipsub(gossipsub::Event::Message {
                ...
                if let Ok(network_message) = serde_json::from_slice::<
                    crate::protocol::messages::NetworkMessage,
                >(&message.data)
                {
                    ...
                    debug!(
                        "Received non-NetworkMessage data: {} bytes",
                        message.data.len()
                    );
                }
            }
    ...
    //     &mut self,
    //     handler: Arc<dyn NetworkEventHandler + Send + Sync>,
    // ) {
    ...
    pub async fn health_check(&self) -> MultivmResult<NetworkHealthReport> {
        ...
        if failed_peers > active_peers * 2 {
            issues.push(format!(
                "High failure rate: {failed_peers} failed vs {active_peers} active"
            ));
            if status == NetworkHealthStatus::Healthy {
                status = NetworkHealthStatus::Warning;
            }
...
impl P2PNetwork {
    ...
    pub async fn send_to_peer(
        &mut self,
        peer_id: String,
        message: crate::protocol::messages::NetworkMessage,
    ) -> MultivmResult<()> {
        // Serialize the message
        let data =
            bincode::serialize(&message).map_err(|e| multivm_common::MultivmError::Network {
    ...
    pub async fn handle_incoming_message(
        ...
        match &message.payload {
            crate::protocol::messages::MessagePayload::Control(_) => {
                debug!("Processing control message from {}", peer_id);
                // Handle control messages (heartbeat, status, etc.)
            }
            crate::protocol::messages::MessagePayload::Discovery(_) => {
                debug!("Processing discovery message from {}", peer_id);
                // Handle peer discovery messages
            }
            crate::protocol::messages::MessagePayload::MultiVm(_) => {
                debug!("Processing MultiVM message from {}", peer_id);
                // Forward to MultiVM layer
            }
            crate::protocol::messages::MessagePayload::Svm(_) => {
                debug!("Processing SVM message from {}", peer_id);
                // Forward to SVM layer
            }
            crate::protocol::messages::MessagePayload::Evm(_) => {
                debug!("Processing EVM message from {}", peer_id);
                // Forward to EVM layer
            }
            crate::protocol::messages::MessagePayload::Custom(_) => {
                debug!("Processing custom message from {}", peer_id);
                // Handle custom/consensus messages
            }
        }
